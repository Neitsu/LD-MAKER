...
export const BarsList: React.FC<{members: MemberState[]}> = ({members}) => {
  ...
  return (
    <div style={{display: 'flex', flexDirection: 'column', gap: 18}}>
      {members.map((m, idx) => {
        const activeBoost = spring({fps, frame, config: {damping: 200}, durationInFrames: 8});
        const scale = m.active ? 1 + 0.03 * activeBoost : 1;
        ...
        return (
          <div
            ...
            style={{
              ...
              filter: m.active ? 'brightness(1.2)' : 'brightness(0.9)',
              opacity: interpolate(frame, [idx * 2, idx * 2 + 8], [0, 1], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'}),
            }}
          >
            ...
            <div style={{width: 720}}>
              ...
              <div style={{height: 20, background: 'rgba(255,255,255,0.14)', borderRadius: 20, marginTop: 8}}>
                <div style={{height: '100%', width, borderRadius: 20, background: m.color, boxShadow: glow}} />
              </div>
            </div>
          </div>
        );
